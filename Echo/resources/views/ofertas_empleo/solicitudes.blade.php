            ...
            @foreach($candidatos_solicitudes as $candidato_solicitud)
                <div class="w-full max-w-xs border border-cyan-300 bg-color-fondo-200 bg-hover-principal b-radius-xl rounded-lg shadow color-texto-blanco">
                    ...
                    <div class="flex flex-col items-center pb-10 color-texto-blanco">
                        <img  class ="w-24 h-24 mb-3 rounded-full shadow-lg" src="{{ asset('storage/img_users/' . $candidato_solicitud->foto) }}" alt="Foto usuario">
                        <h3 class="mb-2 text-2xl font-bold tracking-tight color-texto-blanco">{{$candidato_solicitud ->nombre}}</h4>
                        <div class="flex items-center pr-0.5 py-1">
                            ...
                            <span class="ml-3">{{$candidato_solicitud->email}}</span>
                        </div>
                        <div class="flex mt-4 space-x-3 md:mt-6">
                            <a href="{{ route('candidatos.show', ['id' => $candidato_solicitud->id]) }}" class="btn-principal text-white font-bold cursor-pointer">Ver perfil</a>
